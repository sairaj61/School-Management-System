...
async def get_auto_with_students(auto_id: UUID, db=Depends(get_db)):
    service = AutoManagementService(db)
    return await service.get_auto_with_students(auto_id)


@router.post("/", response_model=AutoManagementResponse)
async def create_auto(auto: AutoManagementCreate, db=Depends(get_db)):
    service = AutoManagementService(db)
...
async def delete_auto(auto_id: UUID, db=Depends(get_db)):
    service = AutoManagementService(db)
...
async def get_all_autos_with_students(db=Depends(get_db)):
    """
