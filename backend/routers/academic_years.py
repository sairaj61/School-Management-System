...
async def get_academic_years(db=Depends(get_db), user: User = Depends(current_active_user)):
    service = AcademicYearService(db)
    return await service.get_all_academic_years()


@router.get("/{year_id}", response_model=AcademicYear)
async def get_academic_year(year_id: UUID, db=Depends(get_db), user: User = Depends(current_active_user)):
    service = AcademicYearService(db)
...
async def delete_academic_year(year_id: UUID, db=Depends(get_db), user: User = Depends(current_active_user)):
    service = AcademicYearService(db)
    return await service.delete_academic_year(year_id)


@router.post("/{year_id}/activate", response_model=AcademicYear)
async def activate_academic_year(year_id: UUID, db=Depends(get_db), user: User = Depends(current_active_user)):
...
async def deactivate_academic_year(year_id: UUID, db=Depends(get_db), user: User = Depends(current_active_user)):
